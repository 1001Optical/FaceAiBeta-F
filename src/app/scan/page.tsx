...
export default function ScanPage() {
    ...
    const handleCapture = async () => {
      ...
      try {
        ...
        const detectRes = await fetch("http://localhost:5001/detect_face_shape", {
          method: "POST",
          headers: {
        ...
        if (!detectRes.ok) {
          throw new Error(`HTTP error! status: ${detectRes.status}`);
        }
        ...
        // 얼굴 인식 실패 처리
        if ((detectData.status === "error" && detectData.status_code === "unable_to_determine") || 
            detectData.shape === "Unknown" || !detectData.shape) {
        ...
        const faceDetail = faceShapeDetails.find((f: FaceShapeDetail) => f.shape === detectData.shape);
        const recommendations = frameRecommendations[detectData.shape as keyof typeof frameRecommendations];
        const frameDetails = recommendations?.recommendedFrames.map((frameName: string) => 
        ...
        const imageUrls = [
          faceDetail?.image,
          ...frameDetails.map((frame: FrameShapeDetail | undefined) => frame?.image)
      ...
      >
        ...
        {step === "loading" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-black bg-opacity-75">
            <Loading />
            <p className="text-white mt-4 text-lg">Analyzing your face shape...</p>
          </div>
        )}
        ...
        {step === "intro" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-10 p-4" style={{background: "rgba(0,0,0,0.35)"}}>
            <div className="max-w-lg text-center text-white flex flex-col gap-8">
              ...
              <p className="mb-8 text-base font-light">
                Click "I'm ready" and ask the customer to face the camera directly, looking straight ahead without any glasses, hats, or masks.
              </p>
              <button
        ...
        {step === "guide" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-10 p-4" style={{background: "rgba(0,0,0,0.35)"}}>
            <h1 className="text-2xl font-bold text-white mb-8 text-center">
              ...
              {countdown && !captured && (
                <span className="block text-lg font-normal mt-2">Capturing in {countdown}...</span>
              )}
            </h1>
            <div
              className="relative flex items-center justify-center"
              style={{ width: "90%", maxWidth: "500px", aspectRatio: "3/4", transform: "scaleX(-1)" }}
            >
              {/* 타원 가이드라인 */}
              <svg
                width="100%"
