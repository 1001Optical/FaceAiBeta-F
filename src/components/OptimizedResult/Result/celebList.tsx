import { CelebType } from '@/types/optimizedResultTypes';
...
const CelebList = ({gender, list, selectCeleb}: IProps) => {
  return <div className={"w-[320px] h-12 px-3 flex flex-col gap-3"}>
    <p className={'heading-xl text-white-600'}>{gender}</p>
    ...
    <div className={"flex flex-col gap-1.5"}>
      {list.map(celeb => (
        <div key={celeb.name} onClick={() => selectCeleb(celeb)} className={"flex gap-2 py-2 items-center cursor-pointer"}>
          ...
          <p className={"w-[224px] heading-md text-white-1000 overflow-hidden text-nowrap text-ellipsis"}>{celeb.name}</p>
        </div>
      ))}
    </div>
  </div>
